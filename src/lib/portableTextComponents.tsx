...
export const portableTextComponents: PortableTextComponents = {
    ...
    image: ({ value }) => {
      ...
      return (
        <div className="relative my-8 overflow-hidden max-w-full flex justify-center">
          <Image
            src={imageUrl}
            alt={value?.alt || value?.altText || "Image"}
            width={400}
            height={400}
  ...
  block: {
    ...
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-[color:var(--primary)] pl-4 italic bg-slate-50/80 py-3 my-4 text-slate-700 rounded-r-md">
        {children}
      </blockquote>
    ),
  },
  marks: {
    ...
    underline: ({ children }) => (
      <u className="underline decoration-[color:var(--primary)]/60 decoration-2 underline-offset-2">
        {children}
      </u>
    ),
    ...
    code: ({ children }) => (
      <code className="bg-slate-100 text-[color:var(--primary-dark)] font-mono px-1.5 py-0.5 rounded text-sm">
        {children}
      </code>
    ),
    link: ({ children, value }) => {
      ...
      if (isExternal) {
        return (
          <a
            href={href}
            className="text-[color:var(--primary)] hover:text-[color:var(--primary-dark)] hover:underline font-semibold transition-colors duration-200"
            target="_blank"
